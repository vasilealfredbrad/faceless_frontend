...
export default function VideoGenerator({ session, onNeedAuth, onVideoGenerated }: Props) {
  ...
  useEffect(() => {
    ...
    const clipsFor = (c: CategoryInfo) => (duration === 30 ? c.clips30 : c.clips60);
    const current = categories.find((c) => c.name === background);
    if (current && clipsFor(current) > 0) return;
    const firstValid = categories.find((c) => clipsFor(c) > 0);
    if (firstValid) setBackground(firstValid.name);
  }, [duration, categories, background]);
            ...
            <>
              ...
              <div>
                ...
                >
                  ...
                  {categories.map((cat) => {
                    const clips = duration === 30 ? cat.clips30 : cat.clips60;
                    return (
                      <option key={cat.name} value={cat.name} disabled={clips === 0}>
                        {cat.name} ({clips} clips)
                      </option>
                    );
                  })}
                </select>
