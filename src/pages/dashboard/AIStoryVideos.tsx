...
function StatusBadge({ status }: { status: string }) {
  ...
  const spin = !["completed", "failed"].includes(status);
  return (
        ...
        <div className="w-full rounded-2xl overflow-hidden bg-black border border-white/10 shadow-2xl">
          {playing && videoUrl ? (
            ...
            <div className="relative cursor-pointer" onClick={() => setPlaying(true)}>
              <img src={job.thumbnail_url} alt={job.topic} className="w-full" style={{ aspectRatio: "9/16", objectFit: "cover" }} />
              <div className="absolute inset-0 flex items-center justify-center">
            ...
            <div className="aspect-[9/16] flex items-center justify-center cursor-pointer" onClick={() => setPlaying(true)}>
        ...
        <div className="w-full mt-3 flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm transition-colors ${
              showDetails ? "bg-primary/20 text-primary border border-primary/30" : "bg-white/5 hover:bg-white/10 text-white/60"
            }`}
          >
        ...
        {showDetails && (
          <div className="w-full mt-3 rounded-2xl bg-surface-card border border-white/10 p-4 space-y-3 overflow-y-auto max-h-[40vh]">
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: Clock, label: "Duration", value: `${job.duration}s` },
                { icon: Mic, label: "Voice", value: job.voice },
                { icon: MonitorPlay, label: "Background", value: job.background },
              ].map(({ icon: Icon, label, value }) => (
                <div key={label} className="rounded-xl bg-surface border border-white/5 p-3">
              ...
              <div className="rounded-xl bg-surface border border-white/5 p-3">
                ...
                <p className="text-xs text-white">
                  {new Date(job.created_at).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                </p>
              </div>
            </div>
            {job.script && (
...
export default function AIStoryVideos({ session }: Props) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "jobs", filter: `user_id=eq.${session.user.id}` }, fetchJobs)
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [session, fetchJobs]);

  const handleVideoGenerated = useCallback(() => { fetchJobs(); }, [fetchJobs]);
  ...
  const usedCount = profile?.daily_videos_used ?? 0;
  const limitCount = Number(appSettings.free_tier_daily_limit ?? 15);
  const isFree = profile?.tier === "free";
  const usagePct = isFree ? Math.min(100, (usedCount / limitCount) * 100) : 0;
    ...
    <div className="flex flex-col xl:flex-row min-h-full">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/35 mt-0.5">Faceless · AI voiceover · Auto subtitles</p>
              </div>
            </div>
            ...
            {isFree && (
              ...
              >
                ...
                <span className="flex items-center gap-0.5 text-[9px] font-semibold text-primary group-hover:text-primary-light transition-colors uppercase tracking-wider">
      ...
      <div className="flex-1 min-w-0 px-6 pt-6 pb-8">
        ...
        {loadingJobs ? (
          ...
          <div className="rounded-2xl border border-dashed border-white/8 flex flex-col items-center justify-center py-20 text-center px-6">
            <Film className="w-10 h-10 text-white/8 mb-4" />
            <p className="text-white/30 text-sm font-medium">No videos yet</p>
            <p className="text-white/15 text-xs mt-1">
              Generate your first video using the form
              <span className="xl:inline hidden"> on the left</span>
              <span className="xl:hidden"> above</span>
            </p>
          </div>
        ) : (
