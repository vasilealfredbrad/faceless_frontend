...
export default function Admin({ session }: Props) {
  ...
  async function handleDelete(sourceId: string) {
    ...
    try {
      await deleteSourceVideo(sourceId);
                            ...
                            {deleteConfirmId === src.id ? (
                              ...
                              >
                                <button
                                  ...
                                  className="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 hover:bg-red-500/30 text-[10px] font-semibold transition-colors disabled:opacity-50"
                                >
                                  {deletingId === src.id ? "..." : "Confirm"}
                                </button>
                                <button
                                  onClick={() => setDeleteConfirmId(null)}
                                  className="px-2 py-1 rounded-lg bg-surface border border-white/10 text-white/40 hover:text-white/60 text-[10px] transition-colors"
                                >
                                  Cancel
                                </button>
                              </div>
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(src.id); }}
                                className="flex-shrink-0 p-1.5 text-white/20 hover:text-red-400 transition-colors"
                                        ...
                                        <Fragment key={clip.id}>
                                          <tr>
                                            <td className="px-3 py-2">
                                              <button
                                                onClick={() => setPreviewClipId(previewClipId === clip.id ? null : clip.id)}
                                                className={`p-1 rounded transition-colors ${previewClipId === clip.id ? "text-accent" : "text-white/20 hover:text-white/60"}`}
                                                title="Preview clip"
                                              >
                                          ...
                                          {previewClipId === clip.id && (
                                            <tr>
                                              <td colSpan={6} className="px-3 pb-3">
                                                <video
                                                  src={`${import.meta.env.VITE_BACKEND_URL || ""}/bg-preview/${src.category}/${clip.clip_duration}/${clip.filename}`}
                                                  controls
                                                  autoPlay
                                                  playsInline
                                                  className="w-full max-w-xs rounded-lg border border-white/10 bg-black"
